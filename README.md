...
После получения представлений вакансий, характеристик OCEAN и кандидатов можно связать кандидатов с вакансиями. Для этого мы рассчитали "степень соответствия" кандидатов  характеристикам OCEAN, а также вакансий тем же чертам по системе OCEAN. Степень соответствия - косинусное расстояние (или корреляция Пирсона), нормированное так, чтобы значения лежали в интервале [0, 1].

- $h$ - размерность скрытого пространства (у нас - 384)
- $V$ - матрица вакансий размера [количество вакансий, h]
- $U$ - матрица кадидатов (пользователей) размера [количество кандидатов, h]
- $O$ - матрица характеристик по системе OCEAN размера [5, h]

Тогда **сходство $s_u$ между кандидатами и характеристиками OCEAN** описывается косинусной близовстью между их векторами:
$$s_u = \text{cosine}(U, O.T)$$

$s_u$ - размерность [количество кандидатов, 5]

Так как мы хотим получить значения в интервале [0, 1] ("насколько в \% данная характеристика выражена у кандидата?"), мы нормировали сходство следующим образом:
$$s_u := (s_u + 1) // 2$$

Аналогично было рассчитано **сходство между вакансиями и характеристиками OCEAN**:
$$s_v = (\text{cosine}(V, O.T) + 1) // 2$$

$s_v$ - размерность [количество кандидатов, 5]

Таким образом, каждая из интересующих сущностей - кандидат и вакансия - связаны с пятью характеристиками OCEAN. Мы предполагаем, что *кандидат и вакансия связаны между собой через типы личности OCEAN* следующим образом:

$$W = (s_u \times s_v.T) + (1 - s_u) \times (1 - s_v.T)$$

$W$ - матрица схожести кандидатов и вакансий, размера [количество кандидатов, количество вакансий]. 

Данная матрица позволяет нам ранжировать по "сходству" как вакансии для кандидатов, так и кандидатов для вакансий.